...
module.exports = function(options) {
  ...
  return util.execFile('php', [
    path.resolve(toolsPath, 'grunt-makepot.php'),
    wpPackage.getType(),
    wpPackage.getPath(),
    wpPackage.getPotFilename(),
    wpPackage.getMainFile().split('.').shift(),
    options.exclude.join(','),
    options.include.join(',')
  ])
  .then(function() {
    var pot = wpPackage.getPot(options.potFile);
    ...
    if (pot.fileExists()) {
      ...
      if (!options.updateTimestamp && !pot.hasChanged()) {
        pot.resetCreationDate();
      }
